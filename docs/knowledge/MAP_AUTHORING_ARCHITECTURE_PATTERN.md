...
The pattern was first validated in the Close Seal production repository on branch `art/first-visual-pass` at commit `072bdfb0d4f2c07f467d044bb2354b50f5d158af` and extended through commit:

`aa2407460e5892c2841079092a665ad4de9e4e03`
...
- `addons/close_seal_map_forge/map_forge_plugin.gd` — editor workspace and direct-authoring controller.
- `addons/close_seal_map_forge/map_forge_canvas.gd` — provider-neutral tactical topology canvas and drag authoring.
...
- `src/prototype/greybox_battlefield.gd` — runtime consumer of the canonical map contract.

The reference workspace is now versioned as **Map Forge 0.3.0**.
...
### 4. Canonical contract → runtime validation

The production prototype now loads its battlefield topology from the same canonical map contract that Map Forge edits. In the observed Close Seal implementation, the runtime reads:

- map bounds;
- base positions;
- hero spawn positions;
- primary route width;
- objective positions/radii.

The real screenshot pipeline therefore exercises not just editor compilation but the downstream contract-to-runtime path. This closes an important failure mode where an editor appears functional while the game continues using unrelated hardcoded coordinates.
...
- Close Seal commit `aa2407460e5892c2841079092a665ad4de9e4e03`

Successful Close Seal workflows at the latest revision:

- `Map Authoring Providers` — run `34745806470` — success
- `Map Contract Integrity` — run `34745806463` — success
- `Closeseal Project Integrity` — run `34745806507` — success
- `Real Prototype Screenshot` — run `34745806469` — success

The screenshot artifact at that revision had digest:

`sha256:2d52da68e30dd8fa7fc782d5d7ba932be2da1b005e324bf314a1418059215248`

Visual inspection confirmed that the map-driven prototype remained framed and coherent: bases/towers were in-bounds, forces remained visible, and the canonical objective rendered in the battlefield center.

## Useful failures observed during validation

### Strict Variant numeric inference

Two development iterations failed under Godot strict warnings because numeric values derived from `Variant` dictionaries were inferred as `Variant`.

The production fix was to make derived numeric state explicit (`float`, `Vector3`, typed dictionaries/arrays where appropriate) and use typed numeric helpers such as `maxf`, `minf`, `absf`, `lerpf` and `clampf`.

This is an example of why the editor integration gate is required even when independent static validation succeeds.

### Hardcoded runtime divergence

Inspection of the initial playable greybox showed that the editor and canonical JSON were not yet the runtime source of truth: battlefield dimensions, bases, spawns and lane dimensions were hardcoded in the prototype.

That divergence was removed by making the prototype consume the canonical map contract. The architecture should therefore explicitly test that authored semantic data reaches runtime, not merely that the editor can read and save it.

## Tactical canvas and direct authoring

The reference editor includes a provider-neutral 2D tactical topology view capable of displaying and editing:

- map bounds;
- bases;
- hero spawns;
- objectives;
- routes and individual route points;
- route widths;
- regions;
- chokepoints.

Observed authoring functions in Map Forge 0.3 include:

- drag-to-move semantic features;
- zoom and pan;
- feature selection;
- width editing;
- add objective;
...
- structural-validation gate before save;
- X-axis competitive symmetry helper;
- formation-width/chokepoint visualization.

Because these functions modify the canonical game-owned data rather than provider internals, they remain useful even if every third-party authoring provider is replaced.
...
10. Make the runtime consume the same canonical contract that the editor writes.
11. Gate saves on contract validity rather than trusting interactive editor state.
12. Keep direct-authoring operations reversible and inspectable in source control.
...
- live two-way synchronization between Terrain3D/Cyclops/ProtonScatter geometry and the semantic contract.
...
- implement and test live provider adapters without making provider data canonical;
