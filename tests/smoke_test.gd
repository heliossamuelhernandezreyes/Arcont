...
func _init()->void:
 ...
 _check_methods("res://scripts/environment_scatter.gd",["_build_chunks","_make_multimesh_cell","_placement_allowed","_terrain_height","_terrain_slope"],failures)
 ...
 _check_text("res://scripts/forest_terrain_relief.gd","village basin + outer ridges + drainage valley + tactical corridors",failures)
 _check_text("res://scripts/environment_scatter.gd","ART-PASS-4-ECO-SCATTER",failures)
 _check_text("res://scripts/environment_scatter.gd","height+slope+mission_clearance",failures)
 ...
 if failures.is_empty():
  print("ARCONT CI: continuous heightmap + ecological chunk scatter + humanoid infected + TPS contract OK");quit(0);return
