...
func _audit_scene(label: String, path: String) -> void:
	if not ResourceLoader.exists(path):
		failures.append("%s missing: %s" % [label, path]); return
	var packed := load(path) as PackedScene
	if packed == null:
		failures.append("%s failed to load as PackedScene: %s" % [label, path]); return
	...
	var size := _visual_bounds(root).size
	...
	if stats.meshes <= 0 or stats.vertices <= 0: failures.append("%s has no usable mesh geometry" % label)
	if stats.skeletons <= 0: failures.append("%s has no Skeleton3D" % label)
	if size.y <= 0.01: failures.append("%s has invalid visual height %.4f" % [label,size.y])
	root.free()
...
func _collect_stats(node: Node, stats: Dictionary) -> void:
	if node is MeshInstance3D:
		...
		stats.skeletons += 1; stats.bones += (node as Skeleton3D).get_bone_count()
	elif node is AnimationPlayer:
		stats.animation_players += 1
		var ap := node as AnimationPlayer
		for library_name in ap.get_animation_library_list(): stats.animations += ap.get_animation_library(library_name).get_animation_list().size()
	for child in node.get_children(): _collect_stats(child,stats)
...
func _visual_bounds(root: Node) -> AABB:
	...
	for mi in meshes:
		...
		var transformed := _transform_aabb(mi.mesh.get_aabb(),mi.transform)
		if not initialized: result=transformed; initialized=true
...
func _transform_aabb(box: AABB, transform: Transform3D) -> AABB:
	var points := [box.position,box.position+Vector3(box.size.x,0,0),box.position+Vector3(0,box.size.y,0),box.position+Vector3(0,0,box.size.z),box.position+Vector3(box.size.x,box.size.y,0),box.position+Vector3(box.size.x,0,box.size.z),box.position+Vector3(0,box.size.y,box.size.z),box.end]
	var first: Vector3 = transform*points[0]
	var min_v := first; var max_v := first
	for point in points:
		var p: Vector3 = transform*point; min_v=min_v.min(p); max_v=max_v.max(p)
	return AABB(min_v,max_v-min_v)

func _audit_forest_folder() -> void:
	var dir := DirAccess.open("res://assets/provisional/cc0_runtime/forest")
	if dir == null: failures.append("forest runtime folder missing"); return
	var count := 0
	for file_name in dir.get_files():
		if file_name.get_extension().to_lower() in ["fbx","glb","gltf"]:
			var path := "res://assets/provisional/cc0_runtime/forest/%s" % file_name
			if ResourceLoader.exists(path) and load(path) is PackedScene: count += 1
	print("RUNTIME_FOREST|loadable_models=%d" % count)
	if count <= 0: failures.append("no loadable forest runtime models")
