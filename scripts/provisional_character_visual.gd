...
func _physics_process(delta:float)->void:
 ...
 if not player_body.is_on_floor():
  ...
  _play_state("run")
  animation_tree.set("parameters/run/run_speed/scale",clampf(speed/maxf(run_reference_speed,0.1),0.65,1.45))
 else:
  _play_state("idle")
...
func _bind_weapon_to_hand()->void:
 ...
 if skeleton==null or skeleton.find_bone(weapon_bone_name)<0:return
 var old_global:=weapon_mount.global_transform
 var attachment:=BoneAttachment3D.new()
 attachment.name="WeaponHandAttachment"
 attachment.bone_name=weapon_bone_name
...
func _update_procedural(delta:float,speed:float)->void:
 ...
 procedural_roll=lerpf(procedural_roll,-lateral*3.0,minf(step*9.0,1.0))
 procedural_pitch=lerpf(procedural_pitch,forward*1.6-(1.8 if aiming else 0.0),minf(step*7.0,1.0))
...
func _setup_animation_tree()->void:
 ...
 var run_scale:=AnimationNodeTimeScale.new()
 var run_tree:=AnimationNodeBlendTree.new()
 run_tree.add_node("run_anim",_animation_node("run"),Vector2(-120,0))
 run_tree.add_node("run_speed",run_scale,Vector2(80,0))
 run_tree.connect_node("run_speed",0,"run_anim")
 run_tree.connect_node("output",0,"run_speed")
 state_machine.add_node("run",run_tree,Vector2(220,0))
 state_machine.add_node("jump",_animation_node("jump"),Vector2(110,-150))
 _connect_state("idle","run",animation_blend)
 _connect_state("run","idle",animation_blend)
 _connect_state("idle","jump",0.08)
 _connect_state("run","jump",0.08)
 _connect_state("jump","idle",0.12)
 _connect_state("jump","run",0.12)
 animation_tree=AnimationTree.new()
 animation_tree.name="LocomotionAnimationTree"
...
func _import_clip(library:AnimationLibrary,path:String,source_name:String,target_name:String,looped:bool)->void:
 ...
 var source:=_find_animation_player(clip_root)
