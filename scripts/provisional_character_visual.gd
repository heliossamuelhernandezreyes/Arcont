...
func _update_procedural(delta:float,speed:float)->void:
 ...
 var target_y:=base_position.y+sin(phase*1.15)*0.006*stability+absf(sin(phase*2.2))*0.005*moving
 ...
 procedural_roll=lerpf(procedural_roll,-lateral*(1.4 if aiming else 2.4),minf(step*9.0,1.0));procedural_pitch=lerpf(procedural_pitch,forward*(0.35 if aiming else 0.9),minf(step*7.0,1.0));position.y=lerpf(position.y,target_y,minf(step*8.0,1.0));rotation_degrees.x=procedural_pitch;rotation_degrees.z=procedural_roll
