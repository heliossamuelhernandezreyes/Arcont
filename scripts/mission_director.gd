...
func _ready() -> void:
	...
	weapon = get_parent().get_node("Player/CameraRig/Camera3D/Weapon")
...
func _update_defense(delta: float) -> void:
	...
	if reinforcement_timer >= reinforcement_interval:
		reinforcement_timer = 0.0; _request_reinforcements(4)
...
func _trigger_defense_events() -> void:
	...
	if elapsed >= 8.0 and not ranged_triggered:
		ranged_triggered = true; info_label.text = "R-3 // CONTACTOS ARMADOS\nUNIDADES XENOLOGICAS TOMAN POSICIONES"; _request_ranged(2)
	if elapsed >= 11.0 and not radio_warning_triggered: radio_warning_triggered = true; info_label.text = "RADIO // MOVIMIENTO MASIVO EN EJE NORTE\nUSE COBERTURA, NO QUEDE FIJO"
	if elapsed >= 15.0 and not brute_triggered: brute_triggered = true; _spawn_brute_event()
	if elapsed >= 21.0 and not xeno_pulse_triggered: xeno_pulse_triggered = true; _trigger_xeno_pulse()
...
func _trigger_xeno_pulse() -> void:
	info_label.text = "RADIO // PULSO XENOLOGICO\nINFECTADOS Y TIRADORES CONVERGIENDO"
	_request_reinforcements(7); _request_ranged(1)
...
func _collect_supplies() -> void:
	...
	_request_reinforcements(8); _request_ranged(2)
	info_label.text = "RADIO // PAQUETE RECUPERADO\nVENTANA DE EXTRACCION ABIERTA"
...
func _complete_mission() -> void:
	stage = STAGE_COMPLETE; objective_label.text = "MISION COMPLETADA // EXTRACCION CONFIRMADA"
	info_label.text = "RADIO // OPERADOR Y R-3 EXTRAIDOS\nDISTRITO 07 PERDIDO"
	mission_completed.emit()
func _set_stage(next_stage: int) -> void:
	stage = next_stage
	if stage == STAGE_DEFEND:
		defend_remaining = defend_duration; reinforcement_timer = 0.0
		blackout_triggered = false; radio_warning_triggered = false; brute_triggered = false; xeno_pulse_triggered = false; ranged_triggered = false
...
func _build_objective_props() -> void:
	generator_root = _root_prop("MissionGenerator", generator_position, true)
	supply_root = _root_prop("MissionSupplies", supply_position, false)
	evac_root = _root_prop("MissionEvac", evac_position, false)
	_build_generator.call_deferred(); _build_supplies.call_deferred(); _build_evac.call_deferred()
...
func _build_generator() -> void:
	...
	generator_light = OmniLight3D.new(); generator_light.position = Vector3(0,1.55,0); generator_light.light_color = Color(1,0.22,0.06); generator_light.light_energy = 2.2; generator_light.omni_range = 5.0; generator_root.add_child(generator_light)
...
func _build_evac() -> void:
	_prop_box(evac_root,Vector3(0,0.08,0),Vector3(4.8,0.16,4.8),Color(0.06,0.14,0.20)); evac_light = OmniLight3D.new(); evac_light.position = Vector3(0,3.2,0); evac_light.light_color = Color(0.14,0.58,1); evac_light.light_energy = 2.8; evac_light.omni_range = 8.0; evac_root.add_child(evac_light)
