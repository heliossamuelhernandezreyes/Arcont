...
func _update_generator(delta:float)->void:
	...
	if distance<=interact_radius:
		...
		generator_progress=maxf(generator_progress-delta*0.35,0.0);objective_label.text="OBJETIVO // REACTIVA EL GENERADOR  •  %.0f m"%distance
...
func _spawn_lancer_event()->void:
	info_label.text="R-3 // FIRMA NO HUMANA CONFIRMADA\nXENO LANCER CARGANDO ARMA ENERGETICA"
	_request_xeno(1)
func _trigger_xeno_pulse()->void:
	info_label.text="RADIO // PULSO XENOLOGICO\nINFECTADOS Y UNIDADES XENO CONVERGIENDO";_request_reinforcements(6);_request_xeno(1)
...
func _collect_supplies()->void:
	...
	_request_reinforcements(8);_request_ranged(1);_request_xeno(1);info_label.text="RADIO // PAQUETE RECUPERADO\nVENTANA DE EXTRACCION ABIERTA";_set_stage(STAGE_EVAC)
...
func _complete_mission()->void:
	stage=STAGE_COMPLETE;objective_label.text="MISION COMPLETADA // EXTRACCION CONFIRMADA";info_label.text="RADIO // OPERADOR Y R-3 EXTRAIDOS\nDISTRITO 07 PERDIDO";mission_completed.emit()
func _set_stage(next_stage:int)->void:
	stage=next_stage
	if stage==STAGE_DEFEND:
		defend_remaining=defend_duration;reinforcement_timer=0.0;blackout_triggered=false;radio_warning_triggered=false;brute_triggered=false;xeno_pulse_triggered=false;ranged_triggered=false;lancer_triggered=false
...
func _build_generator()->void:
	_prop_box(generator_root,Vector3(0,0.55,0),Vector3(1.5,1.1,1.15),Color(0.12,0.16,0.12));generator_light=OmniLight3D.new();generator_light.position=Vector3(0,1.55,0);generator_light.light_color=Color(1,0.22,0.06);generator_light.light_energy=2.2;generator_light.omni_range=5.0;generator_root.add_child(generator_light)
