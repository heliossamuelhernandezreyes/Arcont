...
func _trigger_defense_events()->void:
 ...
 if elapsed>=8.0 and not ranged_triggered:
  ranged_triggered=true
  _request_ranged(2)
 if elapsed>=15.0 and not brute_triggered:
  brute_triggered=true
  _spawn_brute_event()
 if elapsed>=18.0 and not lancer_triggered:
  lancer_triggered=true
  _request_xeno(1)
 if elapsed>=20.5 and not stalker_triggered:
  stalker_triggered=true
  _request_stalker(1)

func _spawn_brute_event()->void:
 if get_parent().has_method("spawn_brute"):
  get_parent().spawn_brute()
...
func _request_ranged(count:int)->void:
 if get_parent().has_method("spawn_ranged_enemies"):
  get_parent().spawn_ranged_enemies(count)
func _request_xeno(count:int)->void:
 if get_parent().has_method("spawn_xeno_lancers"):
  get_parent().spawn_xeno_lancers(count)
func _request_stalker(count:int)->void:
 if get_parent().has_method("spawn_xeno_stalkers"):
  get_parent().spawn_xeno_stalkers(count)
...
func _build_objective_props()->void:
 ...
 generator_light.light_color=Color(1,0.25,0.12)
 generator_light.light_energy=2.5
 generator_light.omni_range=9.0
 ...
 evac_light.light_color=Color(0.15,0.55,1)
 evac_light.light_energy=3.0
 evac_light.omni_range=10.0
