...
func _assign_touch(index:int,position:Vector2)->void:
	if position.distance_to(_fire_center())<=button_radius*1.25 and fire_touch==-1:
		fire_touch=index
		if player.has_method("set_mobile_fire"):player.set_mobile_fire(true)
		return
	...
	if position.x<size.x*0.45 and move_touch==-1:
		move_touch=index;move_origin=position;move_vector=Vector2.ZERO
...
func _release_touch(index:int)->void:
	if index==move_touch:
		...
		fire_touch=-1
		if player and player.has_method("set_mobile_fire"):player.set_mobile_fire(false)
...
func _draw_button(center:Vector2,radius:float,label:String)->void:
	draw_circle(center,radius,Color(0.20,0.24,0.30,0.38));draw_arc(center,radius,0.0,TAU,36,Color(0.86,0.91,1.0,0.55),3.0)
	var font:=ThemeDB.fallback_font;var font_size:=15;var width:=font.get_string_size(label,HORIZONTAL_ALIGNMENT_LEFT,-1,font_size).x
...
func _fire_center()->Vector2:
	var size:=get_viewport_rect().size;return Vector2(size.x-98.0,size.y-132.0)
func _jump_center()->Vector2:
	var size:=get_viewport_rect().size;return Vector2(size.x-210.0,size.y-90.0)
func _reload_center()->Vector2:
	var size:=get_viewport_rect().size;return Vector2(size.x-218.0,size.y-192.0)
func _cover_center()->Vector2:
	var size:=get_viewport_rect().size;return Vector2(size.x-318.0,size.y-156.0)
func _command_center()->Vector2:
	var size:=get_viewport_rect().size;return Vector2(size.x-410.0,size.y-92.0)
func _sprint_center()->Vector2:
	var size:=get_viewport_rect().size;return Vector2(225.0,size.y-245.0)
