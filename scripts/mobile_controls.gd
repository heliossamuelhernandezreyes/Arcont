...
func _ready()->void:
	touch_enabled=force_visible or DisplayServer.is_touchscreen_available();visible=touch_enabled;set_process_input(touch_enabled);call_deferred("_bind_player");queue_redraw()
...
func _assign_touch(index:int,position:Vector2)->void:
	if position.distance_to(_fire_center())<=button_radius*1.25 and fire_touch==-1:fire_touch=index;if weapon and weapon.has_method("set_trigger"):weapon.set_trigger(true);elif player.has_method("set_mobile_fire"):player.set_mobile_fire(true);return
...
func _release_touch(index:int)->void:
	...
	elif index==fire_touch:fire_touch=-1;if weapon and weapon.has_method("set_trigger"):weapon.set_trigger(false);elif player and player.has_method("set_mobile_fire"):player.set_mobile_fire(false)
