...
func _assign_touch(index:int,position:Vector2)->void:
	if position.distance_to(_fire_center())<=button_radius*1.25 and fire_touch==-1:fire_touch=index;if player.has_method("set_mobile_fire"):player.set_mobile_fire(true);return
...
func _release_touch(index:int)->void:
	...
	elif index==fire_touch:fire_touch=-1;if player and player.has_method("set_mobile_fire"):player.set_mobile_fire(false)
