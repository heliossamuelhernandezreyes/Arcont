...
func _ready() -> void:
	...
	if scene: awareness = scene.get_node_or_null("AwarenessDirector")
...
func set_command(next_command: String) -> void:
	var normalized := next_command.to_upper()
	if normalized not in ["FOLLOW", "HOLD", "ASSAULT"]:
		return
	command = normalized
	if command == "HOLD": hold_position = global_position
	status_light.light_color = _command_color(command)
...
func _physics_process(delta: float) -> void:
	if down:
		reboot_timer -= delta
		if reboot_timer <= 0.0: activate_unit()
		return
...
func _update_follow(delta: float) -> void:
	...
	if command == "HOLD":
		var to_hold := hold_position-global_position;to_hold.y=0.0
		if to_hold.length()>0.55: desired=to_hold.normalized()*move_speed*0.8
	elif command == "ASSAULT" and target != null and is_instance_valid(target):
		var to_enemy := target.global_position-global_position;to_enemy.y=0.0
		if to_enemy.length()>fire_range*0.62: desired=to_enemy.normalized()*move_speed
	else:
		var offset := player.global_position-global_position;offset.y=0.0
		var distance := offset.length()
		if distance>teleport_distance and command=="FOLLOW":
			global_position=player.global_position+Vector3(-2.0,0.2,2.0);velocity=Vector3.ZERO;return
		if distance>follow_distance: desired=offset.normalized()*move_speed
	velocity.x=move_toward(velocity.x,desired.x,acceleration*delta)
	velocity.z=move_toward(velocity.z,desired.z,acceleration*delta)
	if desired.length_squared()>0.01:look_at(global_position+Vector3(desired.x,0.0,desired.z),Vector3.UP)

func _update_combat() -> void:
	target=_nearest_enemy()
	if target==null:return
	var distance:=global_position.distance_to(target.global_position)
	if distance>fire_range:return
...
func _nearest_enemy() -> Node3D:
	var best:Node3D
	var best_distance:=scan_range*(1.35 if command=="ASSAULT" else 1.0)
...
func _has_line_of_sight(candidate:Node3D,aim_position:Vector3)->bool:
	...
	var query:=PhysicsRayQueryParameters3D.create(origin,aim_position);query.exclude=[get_rid()];query.collide_with_areas=false
...
func _fire_at(candidate:Node3D,aim_position:Vector3)->void:
	...
	if candidate.has_method("apply_hit"):candidate.apply_hit(aim_position,direction,shot_damage,"robot",impact_force)
...
func _restore_status_light()->void:
	if is_instance_valid(status_light):status_light.light_energy=1.2 if active and not down else 0.15

func apply_damage(amount:float,_push_direction:=Vector3.ZERO)->void:
	if down or not active:return
	health=maxf(health-amount,0.0);health_changed.emit(health,max_health)
	if health<=0.0:_down_unit()
...
func _down_unit()->void:
	down=true;active=false;reboot_timer=reboot_delay;velocity=Vector3.ZERO;collision.set_deferred("disabled",true)
	status_light.light_color=Color(1.0,0.08,0.04);status_light.light_energy=0.4;body_mesh.rotation_degrees.z=80.0;downed.emit()

func _set_active_visual(value:bool)->void:
	body_mesh.rotation=Vector3.ZERO
	status_light.light_color=_command_color(command) if value else Color(0.18,0.20,0.22)
