...
func _spawn_blood(hit_point: Vector3, hit_normal: Vector3) -> void:
	...
	sphere.height = 0.16 * visual_scale
	puff.mesh = sphere
	var mat := StandardMaterial3D.new()
	mat.albedo_color = Color(0.35, 0.015, 0.02, 0.85)
	mat.transparency = BaseMaterial3D.TRANSPARENCY_ALPHA
	puff.material_override = mat
	root.add_child(puff)
	puff.global_position = hit_point + hit_normal * 0.025
	var tween := puff.create_tween()
	tween.tween_property(puff, "scale", Vector3.ONE * 2.6, blood_lifetime)
	tween.parallel().tween_property(mat, "albedo_color:a", 0.0, blood_lifetime)
...
func _spawn_impact_decal(hit_point: Vector3, hit_normal: Vector3) -> void:
	...
	mark.mesh = quad
	var mat := StandardMaterial3D.new()
	mat.albedo_color = Color(0.06, 0.06, 0.06, 0.9)
