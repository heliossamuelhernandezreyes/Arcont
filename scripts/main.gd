...
func _ready() -> void:
	...
	player.health_changed.connect(_on_player_health_changed); player.died.connect(_on_player_died)
	weapon.ammo_changed.connect(_on_ammo_changed); weapon.reload_state_changed.connect(_on_reload_state_changed)
	weapon.shot_fired.connect(feedback.on_shot_fired); weapon.impact_feedback.connect(feedback.on_hit_feedback)
	...
	_collect_spawn_points(); _on_player_health_changed(player.health,player.max_health); _on_ammo_changed(weapon.ammo_in_mag,weapon.reserve_ammo,weapon.magazine_size); _start_next_wave()
...
func _start_next_wave() -> void:
	...
	wave += 1
	var count := mini(first_wave_size+(wave-1)*wave_growth,_enemy_cap())
	alive = count
...
func spawn_ranged_enemies(requested_count: int) -> void:
	...
	var count:=mini(requested_count,maxi(_enemy_cap()-alive,0))
	for i in count:
		var ranged:=RANGED_SCENE.instantiate(); ranged.died.connect(_on_special_enemy_died); $Enemies.add_child(ranged)
		ranged.global_position=_spawn_position_for(alive+i+5,maxi(alive+count,1))
...
func _on_enemy_died(enemy:Node)->void:
	alive=maxi(alive-1,0); enemy.deactivate()
	if not enemy_pool.has(enemy):enemy_pool.append(enemy)
	_update_hud(); _check_wave_clear()
...
func _check_wave_clear()->void:
	if alive==0 and not waiting_for_next_wave and not game_over and not mission_complete:
		waiting_for_next_wave=true; await get_tree().create_timer(time_between_waves).timeout; waiting_for_next_wave=false; _start_next_wave()
