...
func spawn_reinforcements(requested_count: int) -> void:
	...
	alive += count
	for i in count: _spawn_enemy(start_index + i, alive)
	_update_hud()
...
func _spawn_enemy(index: int, total: int) -> void:
	...
	if enemy.has_method("activate"): enemy.activate(spawn_position, player, budget.ai_interval, budget.gore_parts)
	else: enemy.position = spawn_position
func _spawn_position_for(index: int, total: int) -> Vector3:
	if not spawn_points.is_empty():
		...
		return spawn_points[point_index].global_position + Vector3(randf_range(-1.1,1.1),0,randf_range(-1.1,1.1))
...
func _on_enemy_died(enemy: Node) -> void:
	...
	if enemy.has_method("deactivate"): enemy.deactivate()
...
func _on_mission_completed() -> void:
	mission_complete = true; waiting_for_next_wave = false; wave_label.text = "EXTRACCION"
