...
@export var building_visibility_range:=185.0
@export var vehicle_visibility_range:=95.0
@export var prop_visibility_range:=70.0
...
func _build_city()->void:
 ...
 for d in placements:
  var pos:=Vector3(float(d[0]),0.15,float(d[1]));var asset:=_spawn_asset(BUILDINGS[int(d[2])],pos,Vector3(0,90 if pos.x<0 else -90,0),Vector3.ONE,building_visibility_range)
...
func _vehicle(pos:Vector3,rot:Vector3)->void:
 _collision_box("VehicleCollision",pos+Vector3(0,0.65,0),Vector3(2.0,1.3,4.2),rot);_spawn_asset(CAR_MODELS[vehicle_index%CAR_MODELS.size()],pos,rot,Vector3.ONE,vehicle_visibility_range);vehicle_index+=1
func _build_props()->void:
 for p in [Vector3(-9,0.15,-5),Vector3(9,0.15,18),Vector3(-9,0.15,38),Vector3(9,0.15,-37)]:_spawn_asset(PROP_MODELS["bench"],p,Vector3(0,90,0),Vector3.ONE,prop_visibility_range)
 for p in [Vector3(6,0.15,8),Vector3(-7,0.15,31),Vector3(8,0.15,-28)]:_spawn_asset(PROP_MODELS["box_a"],p,Vector3.ZERO,Vector3.ONE,prop_visibility_range)
...
func _spawn_asset(path:String,pos:Vector3,rot:=Vector3.ZERO,scale_value:=Vector3.ONE,visibility_end:=0.0)->Node3D:
 ...
 instance.position=pos;instance.rotation_degrees=rot;instance.scale=scale_value;add_child(instance)
...
func _apply_visibility_range(node:Node,end_distance:float)->void:
 if node is GeometryInstance3D:
  var geometry:=node as GeometryInstance3D;geometry.visibility_range_end=end_distance;geometry.visibility_range_end_margin=minf(12.0,end_distance*0.12);geometry.visibility_range_fade_mode=GeometryInstance3D.VISIBILITY_RANGE_FADE_SELF
