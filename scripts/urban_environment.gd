...
func _build_ground() -> void:
	...
	# Sidewalk strips frame the main north/south avenue.
...
func _build_city_blocks() -> void:
	# West side: tall shells with deliberate alleys between them.
	...
	# East side, less regular to create sightline variation.
	...
	# Low service structures near the playable center.
...
func _build_centerpiece() -> void:
	# Collapsed checkpoint / evacuation island in the center of the boulevard.
	...
	# Rubble creates partial cover without sealing the arena.
	for data in [
...
func _build_barricades() -> void:
	# Barricades deliberately leave gaps so direct-chase AI cannot become trapped.
	...
	_box("BarricadeW", Vector3(-13.2, 0.55, 5.5), Vector3(0.55, 1.1, 4.0), mat_military, true, Vector3(0, 0, 0))
	_box("BarricadeE", Vector3(13.0, 0.55, 3.2), Vector3(0.55, 1.1, 4.2), mat_military, true, Vector3(0, 0, 0))
...
func _build_alien_incursion() -> void:
	# A small corrupted zone hints at the invasion without requiring final alien assets.
	for data in [
	...
	light.name = "AlienGlow"
	light.position = Vector3(8.8, 2.3, 11.2)
	light.light_color = Color(0.66, 0.12, 1.0)
...
func _build_landmarks() -> void:
	# Evac beacon and a broken overhead sign provide navigation landmarks.
	var beacon := OmniLight3D.new()
	beacon.position = Vector3(-0.5, 4.2, -29.0)
	beacon.light_color = Color(0.15, 0.55, 1.0)
...
func _build_spawn_points() -> void:
	var points := [
		Vector3(-4.0, 0.2, -32.0), Vector3(4.5, 0.2, -32.0),
		Vector3(-4.2, 0.2, 32.0), Vector3(4.0, 0.2, 32.0),
		Vector3(-25.0, 0.2, 4.0), Vector3(-25.0, 0.2, 8.0),
		Vector3(25.0, 0.2, 2.0), Vector3(25.0, 0.2, 7.0),
		Vector3(-11.0, 0.2, -29.0), Vector3(12.0, 0.2, 29.0)
	]
	for i in points.size():
		var marker := Marker3D.new()
		marker.name = "EnemySpawn%02d" % i
...
func _building(position: Vector3, size: Vector3, material: Material) -> void:
	...
	# Simple facade strips give scale/depth with negligible geometry cost.
