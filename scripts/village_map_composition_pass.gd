...
func _fence_line(center: Vector3, span: Vector3, leave_gate: bool) -> void:
 ...
 var step: float = 2.4
 var count: int = maxi(2,int(floor(length/step)))
 for i: int in range(count+1):
  var t: float = float(i)/float(count)-0.5
  if leave_gate and absf(t) < 0.15:
   continue
  ...
  if i < count:
   ...
   if leave_gate and (absf(t)<0.15 or absf(next_t)<0.15):
    continue
   ...
   var size := Vector3(seg_len,0.10,0.10) if along_x else Vector3(0.10,0.10,seg_len)
   _box("FenceRail",Vector3(mid.x,my+0.82,mid.z),size,mat_wood)
   _box("FenceRail",Vector3(mid.x,my+0.38,mid.z),size,mat_wood)
...
func _terrain_patch(name_text: String, center: Vector3, size: Vector2, material: Material, offset: float) -> void:
 ...
 _tri(st,p[0],p[1],p[2]); _tri(st,p[0],p[2],p[3])
 st.generate_normals()
 var mesh := st.commit()
 ...
 node.set_meta("art_layer","parcel_ground")
 node.set_meta("terrain_grounded",true)
 add_child(node)

func _tri(st: SurfaceTool,a: Vector3,b: Vector3,c: Vector3) -> void:
 st.set_uv(Vector2(a.x,a.z)*0.15); st.add_vertex(a)
...
func _box(name_text: String,pos: Vector3,size: Vector3,material: Material,rot := Vector3.ZERO) -> MeshInstance3D:
 ...
 node.set_meta("art_layer","village_composition")
 add_child(node)
 return node
