...
func _ready() -> void:
	...
	desired_camera_position = camera.position
...
func _physics_process(delta: float) -> void:
	if dead: return
	_handle_gamepad_look(delta); _update_cover_state()
	...
	var pad_move := _gamepad_move(); var input_vec := keyboard_move
	...
	var cam_forward := -camera_rig.global_transform.basis.z; var cam_right := camera_rig.global_transform.basis.x
	...
	if in_cover:
		...
		var tangent := Vector3.UP.cross(cover_normal).normalized(); direction = tangent*input_vec.x
...
func _toggle_cover() -> void:
	if in_cover: in_cover=false; return
	cover_probe.force_raycast_update()
	if cover_probe.is_colliding(): in_cover=true; cover_normal=cover_probe.get_collision_normal()
func _update_cover_state() -> void:
	if not in_cover: return
	cover_probe.force_raycast_update()
	if not cover_probe.is_colliding(): in_cover=false; return
	cover_normal=cover_probe.get_collision_normal(); body_visual.look_at(body_visual.global_position-cover_normal,Vector3.UP)
func _switch_shoulder() -> void:
	shoulder_side *= -1.0; desired_camera_position.x = 0.72*shoulder_side
func apply_damage(amount: float, source_direction:=Vector3.ZERO, zone:="torso", penetration:=1.0) -> void:
	...
	health=maxf(health-final_damage,0.0); injuries[normalized_zone]=clamp(float(injuries[normalized_zone])+final_damage/70.0,0.0,1.0)
...
func respawn(at_position: Vector3) -> void:
	global_position=at_position; health=max_health; dead=false; in_cover=false
...
func _on_weapon_recoil(recoil_pitch: float,recoil_yaw: float)->void:
	rotate_y(recoil_yaw); pitch=clamp(pitch+recoil_pitch,deg_to_rad(-65.0),deg_to_rad(55.0)); camera_rig.rotation.x=pitch
