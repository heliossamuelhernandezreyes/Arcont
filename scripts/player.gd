...
var desired_camera_position:=Vector3.ZERO
...
func _update_cover_camera()->void:pass
...
func _update_camera_collision()->void:pass
...
func add_mobile_look(delta:Vector2)->void:
 if dead:return
 var adjusted:=delta
 if mobile_invert_y:adjusted.y=-adjusted.y
 _apply_look(adjusted*mobile_look_sensitivity)
func set_mobile_fire(active:bool)->void:mobile_fire=active and not dead
...
func request_reload()->void:
 if not dead and weapon and weapon.has_method("request_reload"):weapon.request_reload()
func _try_fire()->void:
 if not dead and weapon and weapon.has_method("try_fire"):weapon.try_fire()
func _on_weapon_recoil(recoil_pitch_value:float,recoil_yaw_value:float)->void:
 rotate_y(recoil_yaw_value);pitch=clampf(pitch+recoil_pitch_value,deg_to_rad(-65.0),deg_to_rad(55.0));camera_rig.rotation.x=pitch
func _apply_look(amount:Vector2)->void:
 rotate_y(-amount.x);pitch=clampf(pitch-amount.y,deg_to_rad(-65.0),deg_to_rad(55.0));camera_rig.rotation.x=pitch
