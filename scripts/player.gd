...
func _ready()->void:
	add_to_group("player");health=max_health;camera_rig.rotation.x=pitch;base_camera_position=camera.position;desired_camera_position=base_camera_position;health_changed.emit(health,max_health)
...
func _physics_process(delta:float)->void:
	...
	if mobility and mobility.has_method("record_vertical_speed") and not currently_on_floor:mobility.record_vertical_speed(velocity.y)
	if currently_on_floor and not was_on_floor and mobility and mobility.has_method("handle_landing"):mobility.handle_landing()
	was_on_floor=currently_on_floor
	if mobility and mobility.has_method("blocks_normal_movement") and mobility.blocks_normal_movement():_update_camera_collision();return
	if not is_on_floor():velocity.y-=gravity*delta
		...
		if not traversed and not (mobility and mobility.has_method("blocks_jump") and mobility.blocks_jump()):
			var jump_penalty:=clampf(1.0-_leg_injury_load()*0.18,0.62,1.0);velocity.y=jump_velocity*jump_penalty
	...
	if in_cover:
		...
		var tangent:=Vector3.UP.cross(cover_normal).normalized();direction=tangent*input_vec.x;cover_peek_side=_cover_edge_peek(input_vec.x)
	...
	else:cover_peek_side=0.0;_update_cover_camera()
	...
	move_and_slide();_update_camera_collision()
...
func _world_direction(input_vec:Vector2)->Vector3:
	var cam_forward:=-camera_rig.global_transform.basis.z;var cam_right:=camera_rig.global_transform.basis.x;cam_forward.y=0.0;cam_right.y=0.0
	return (cam_right.normalized()*input_vec.x+cam_forward.normalized()*-input_vec.y).normalized()
...
func get_weapon_spread_multiplier()->float:
	var arm_load:=(float(injuries["left_arm"])+float(injuries["right_arm"]))*0.5;return 1.0+arm_load*0.72+suppression*0.62
func get_recoil_multiplier()->float:
	var arm_load:=(float(injuries["left_arm"])+float(injuries["right_arm"]))*0.5;return 1.0+arm_load*0.58+suppression*0.48
func get_reload_time_multiplier()->float:
	var arm_load:=(float(injuries["left_arm"])+float(injuries["right_arm"]))*0.5;return 1.0+arm_load*0.55
...
func _enter_cover(collider:Object,normal:Vector3)->void:
	in_cover=true;cover_collider=collider;cover_normal=normal.normalized();cover_height=Ballistics.cover_height_for(collider);var world:=get_world_3d()
	if world:
...
func _update_cover_state()->void:
	...
	if cover_probe.is_colliding():
		...
		if collider==cover_collider or cover_collider==null:cover_collider=collider;cover_normal=cover_probe.get_collision_normal().normalized();cover_height=Ballistics.cover_height_for(collider);body_visual.look_at(body_visual.global_position-cover_normal,Vector3.UP);return
...
func _try_transition_to_adjacent_cover(preferred_direction:Vector3)->bool:
	...
	return false
func _switch_shoulder()->void:shoulder_side*=-1.0;_update_cover_camera()
func _update_camera_collision()->void:
	var world:=get_world_3d();if world==null:return
	var origin:=camera_rig.global_position;var desired_global:=camera_rig.to_global(desired_camera_position);var query:=PhysicsRayQueryParameters3D.create(origin,desired_global);query.exclude=[get_rid()];query.collide_with_areas=false;var hit:=world.direct_space_state.intersect_ray(query)
...
func apply_damage(amount:float,source_direction:=Vector3.ZERO,zone:="torso",penetration:=1.0)->void:
	...
	var normalized_zone:=zone if injuries.has(zone) else "torso";var armor_effect:=armor_rating if normalized_zone=="torso" else armor_rating*0.35;var final_damage:float=amount*maxf(0.2,penetration)*(1.0-armor_effect)
	if normalized_zone=="head":final_damage*=1.75
	health=maxf(health-final_damage,0.0);injuries[normalized_zone]=clamp(float(injuries[normalized_zone])+final_damage/70.0,0.0,1.0);injury_changed.emit(normalized_zone,float(injuries[normalized_zone]));health_changed.emit(health,max_health);apply_suppression(clampf(final_damage/45.0,0.12,0.75),0.7)
...
func request_crouch()->void:
	if not dead and mobility and mobility.has_method("request_crouch_or_slide"):mobility.request_crouch_or_slide()
func request_slide()->void:request_crouch()
...
func _on_weapon_recoil(recoil_pitch_value:float,recoil_yaw_value:float)->void:rotate_y(recoil_yaw_value);pitch=clamp(pitch+recoil_pitch_value,deg_to_rad(-65.0),deg_to_rad(55.0));camera_rig.rotation.x=pitch
func _apply_look(amount:Vector2)->void:rotate_y(-amount.x);pitch=clamp(pitch-amount.y,deg_to_rad(-65.0),deg_to_rad(55.0));camera_rig.rotation.x=pitch
