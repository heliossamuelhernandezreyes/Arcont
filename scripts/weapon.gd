...
var reserve_by_slot:=[48,150,75,30]
var fire_timer:=0.0
...
func try_fire()->bool:
 ...
 var ads_recoil:=0.78 if aiming else 1.0;recoil_requested.emit(float(p["recoil_pitch"])*recoil_mult*ads_recoil,randf_range(-float(p["recoil_yaw"]),float(p["recoil_yaw"]))*recoil_mult*ads_recoil);_fire_weapon();return true
func _report_weapon_sound()->void:
 var scene:=get_tree().current_scene;if scene==null:return
 var awareness:=scene.get_node_or_null("AwarenessDirector");if awareness and awareness.has_method("report_sound"):awareness.report_sound(camera.global_position,float(profiles[slot]["noise"]),weapon_name.to_lower())
func _fire_weapon()->void:
 ...
 var spread:=tan(deg_to_rad(float(p["spread"])*spread_mult));var player:=handling as CollisionObject3D;var exclude:Array[RID]=[];if player:exclude.append(player.get_rid())
...
func _build_weapon_visual()->void:
 ...
 var steel:=_mat(Color(0.07,0.085,0.095),0.74,0.48);var dark:=_mat(Color(0.025,0.03,0.034),0.82,0.34);var polymer:=_mat(Color(0.10,0.115,0.105),0.12,0.82);var accent:=_mat(Color(0.34,0.16,0.055),0.18,0.68)
 if slot==0:_add_box(gun,"DetailReceiver",Vector3.ZERO,Vector3(0.26,0.19,0.58),steel);_add_box(gun,"DetailStock",Vector3(0,-0.015,0.42),Vector3(0.24,0.17,0.34),polymer);_add_cylinder(gun,"DetailBarrel",Vector3(0,0.035,-0.62),0.055,0.96,dark);_add_box(gun,"DetailPump",Vector3(0,-0.075,-0.35),Vector3(0.20,0.15,0.31),accent)
 elif slot==1:_add_box(gun,"DetailReceiver",Vector3.ZERO,Vector3(0.23,0.18,0.50),steel);_add_box(gun,"DetailMagazine",Vector3(0,-0.20,-0.05),Vector3(0.15,0.34,0.18),dark);_add_cylinder(gun,"DetailBarrel",Vector3(0,0.03,-0.58),0.038,0.82,dark)
 elif slot==2:_add_box(gun,"DetailSlide",Vector3(0,0.03,-0.12),Vector3(0.18,0.13,0.42),steel);_add_box(gun,"DetailGrip",Vector3(0,-0.16,0.08),Vector3(0.15,0.30,0.18),polymer);_add_cylinder(gun,"DetailBarrel",Vector3(0,0.03,-0.34),0.028,0.30,dark)
 else:_add_box(gun,"DetailReceiver",Vector3.ZERO,Vector3(0.20,0.17,0.62),steel);_add_box(gun,"DetailStock",Vector3(0,-0.02,0.48),Vector3(0.21,0.17,0.48),accent);_add_cylinder(gun,"DetailLongBarrel",Vector3(0,0.03,-0.82),0.034,1.25,dark);_add_cylinder(gun,"DetailScope",Vector3(0,0.16,-0.12),0.055,0.42,dark);_add_box(gun,"DetailBolt",Vector3(0.15,0.04,0.02),Vector3(0.18,0.05,0.06),steel)
...
func _add_box(parent:Node3D,node_name:String,position:Vector3,size:Vector3,material:Material,rotation_deg:=Vector3.ZERO)->void:var node:=MeshInstance3D.new();node.name=node_name;node.position=position;node.rotation_degrees=rotation_deg;var mesh:=BoxMesh.new();mesh.size=size;node.mesh=mesh;node.material_override=material;parent.add_child(node)
func _add_cylinder(parent:Node3D,node_name:String,position:Vector3,radius:float,length:float,material:Material)->void:var node:=MeshInstance3D.new();node.name=node_name;node.position=position;node.rotation_degrees=Vector3(90,0,0);var mesh:=CylinderMesh.new();mesh.top_radius=radius;mesh.bottom_radius=radius;mesh.height=length;node.mesh=mesh;node.material_override=material;parent.add_child(node)
