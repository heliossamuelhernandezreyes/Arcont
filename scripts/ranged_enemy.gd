...
func _ready() -> void:
	...
	add_to_group("ranged_enemy"); add_to_group("enemies_active")
func _physics_process(delta: float) -> void:
	...
	var flat:=player.global_position-global_position;flat.y=0.0;var distance:=flat.length();var desired:=Vector3.ZERO
	...
	velocity.x=move_toward(velocity.x,desired.x,8.0*delta);velocity.z=move_toward(velocity.z,desired.z,8.0*delta)
...
func _trace_round(space_state:PhysicsDirectSpaceState3D,origin:Vector3,direction:Vector3)->void:
	...
	for _pass in 2:
		...
		if collider==player and player.has_method("apply_damage"):
			player.apply_damage(shot_damage*Ballistics.damage_scale(energy,penetration_energy),direction,_pick_hit_zone(),Ballistics.damage_scale(energy,penetration_energy));return
...
func apply_hit(_hit_point:Vector3,direction:Vector3,amount:float,_weapon_type:="shotgun",impact_force:=1.0)->void:
	...
	if health<=0.0:active=false;remove_from_group("enemies_active");died.emit(self);queue_free()
