...
func _on_weapon_changed(_name:String,_slot:int)->void:
	recoil_time=0.0;bolt_cycle_time=0.0;reload_feedback_stage=-1;reload_feedback_strength=0.0
...
func _on_shot()->void:
	...
	if current_slot==3:
		recoil_duration=0.24;recoil_time=recoil_duration;bolt_cycle_duration=1.35;bolt_cycle_time=bolt_cycle_duration
		return
...
func _ensure_round_visual()->void:
	...
	round_visual=MeshInstance3D.new();round_visual.name="DetailRound"
	var mesh:=CylinderMesh.new();mesh.top_radius=0.022;mesh.bottom_radius=0.025;mesh.height=0.12;mesh.radial_segments=10;round_visual.mesh=mesh
	var material:=StandardMaterial3D.new();material.albedo_color=Color(0.62,0.38,0.12);material.metallic=0.72;material.roughness=0.28;round_visual.material_override=material
	round_visual.position=Vector3(0.0,0.12,0.02);round_visual.rotation_degrees=Vector3(88.0,0.0,0.0);round_visual.visible=false;gun.add_child(round_visual)
	_capture_detail_bases()
