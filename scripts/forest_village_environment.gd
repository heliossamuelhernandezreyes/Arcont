...
@export var tree_count := 92
@export var bush_count := 56
...
func _ready() -> void:
 ...
 _spawn_points()
 _bind_budget()
 set_meta("art_status", "ART-PASS-1")
 set_meta("environment_type", "forest_village")

func _materials() -> void:
 mat_soil = _mat(Color(0.115,0.105,0.075), 0.98)
 mat_path = _mat(Color(0.25,0.205,0.135), 0.96)
 mat_water = _mat(Color(0.055,0.14,0.17), 0.55)
 mat_wood = _mat(Color(0.20,0.125,0.065), 0.94)
 mat_stone = _mat(Color(0.24,0.255,0.25), 0.98)
 mat_roof = _mat(Color(0.20,0.075,0.055), 0.91)
 mat_dark = _mat(Color(0.045,0.055,0.052), 0.98)
...
func _ground_and_paths() -> void:
 ...
 # Main muddy road deliberately bends through the settlement to break long sightlines.
 ...
 _box("WestTrack", Vector3(-31,0.012,4), Vector3(54,0.03,5.5), mat_path, false, Vector3(0,-8,0))
 _box("EastTrack", Vector3(31,0.012,-7), Vector3(52,0.03,5.0), mat_path, false, Vector3(0,12,0))
 # Shallow stream creates a visual/navigation seam and a bridge landmark.
 _box("Stream", Vector3(-37,-0.03,-40), Vector3(72,0.08,5.5), mat_water, false, Vector3(0,-7,0))
 _spawn_asset(BRIDGE, Vector3(-4,0.12,-35), Vector3(0,83,0), 120.0, "landmark", 7.0)
...
func _village() -> void:
 ...
 for i in range(homes.size()):
  var data: Array = homes[i]
  var pos: Vector3 = data[0]
  _spawn_asset(BUILDINGS[int(data[2])], pos, Vector3(0,float(data[1]),0), 170.0, "village", 13.5)
  _collision_box("HouseCollision", pos + Vector3(0,3.6,0), Vector3(11.5,7.2,11.5), Vector3(0,float(data[1]),0))
 # Central square: generator defense arena, church/bell-tower silhouette and cover rhythm.
 _box("VillageSquare", Vector3(0,0.02,24), Vector3(30,0.05,25), mat_path, false)
 ...
 for p in [Vector3(-5,0.55,30),Vector3(5,0.55,28),Vector3(-4,0.55,16),Vector3(6,0.55,18)]:
  _box("VillageCover", p, Vector3(2.6,1.1,0.75), mat_wood, true, Vector3(0,rng.randf_range(-25,25),0))
 for i in range(5):
  _spawn_asset(BOX, Vector3(-6.0 + i*3.0,0.15,34.0 + float(i%2)), Vector3(0,float(i*27),0), 60.0, "prop", 1.0)
 # Abandoned vehicles tell the evacuation story without turning the village into a city.
...
func _forest_mass() -> void:
 # Four spatial chunks. Godot culls each chunk independently; future pass can convert each family to MultiMesh.
 ...
 for chunk_index in range(chunks.size()):
  ...
  for i in range(per_chunk):
   var pos := Vector3(center.x + rng.randf_range(-28,28),0.08,center.y + rng.randf_range(-38,38))
   if _reserved_clearance(pos):
    continue
   var tree := _spawn_asset_to(chunk, TREE, pos, Vector3(0,rng.randf_range(0,360),0), 150.0, "canopy", rng.randf_range(8.5,12.5))
   if tree:
...
func _forest_detail() -> void:
 for i in range(bush_count):
  ...
  if not _reserved_clearance(p):
   _spawn_asset(BUSH,p,Vector3(0,rng.randf_range(0,360),0),72.0,"understory",rng.randf_range(1.1,1.8))
 for i in range(grass_count):
  var p := _random_outer_position()
  if not _reserved_clearance(p):
   _spawn_asset(GRASS,p,Vector3(0,rng.randf_range(0,360),0),46.0,"ground_detail",rng.randf_range(0.65,1.1))
 for i in range(16):
  var p := _random_outer_position()
  if not _reserved_clearance(p):
   _spawn_asset(STUMP,p,Vector3(0,rng.randf_range(0,360),0),70.0,"prop",rng.randf_range(0.7,1.15))
 for p in [Vector3(-57,0,-18),Vector3(54,0,19),Vector3(-49,0,58),Vector3(48,0,-62),Vector3(-63,0,-72),Vector3(61,0,72)]:
  _spawn_asset(ROCK,p,Vector3(0,rng.randf_range(0,360),0),145.0,"landform",rng.randf_range(7.0,11.0))

func _story_props() -> void:
 # Campfire uses real CC0 logs now; flame VFX will be layered once sprite pack is promoted.
 for p in [Vector3(11,0.12,25),Vector3(-31,0.12,-4)]:
  _spawn_asset(CAMPFIRE,p,Vector3(0,rng.randf_range(0,360),0),65.0,"story_prop",2.0)
  ...
  add_child(glow)
 # Fallen barricades and dark patches guide the eye toward the generator square.
 for data in [[Vector3(-2,0.25,5),18.0],[Vector3(5,0.25,44),-24.0],[Vector3(-6,0.25,-18),31.0]]:
  _box("FallenBarricade",data[0],Vector3(3.2,0.5,0.55),mat_wood,true,Vector3(0,float(data[1]),0))
 for p in [Vector3(2,0.025,12),Vector3(-4,0.025,40),Vector3(5,0.025,-22)]:
  _box("ScorchedGround",p,Vector3(3.2,0.018,4.5),mat_dark,false,Vector3(0,rng.randf_range(-25,25),0))

func _reserved_clearance(pos: Vector3) -> bool:
 # Keep central village and main north/south road readable for combat.
 if absf(pos.x) < 43.0 and pos.z > -48.0 and pos.z < 55.0:
  return true
 if absf(pos.x) < 9.0:
  return true
 return false
...
func _spawn_points() -> void:
 var points := [Vector3(-60,0.2,-78),Vector3(58,0.2,-75),Vector3(-66,0.2,-20),Vector3(65,0.2,-15),Vector3(-62,0.2,55),Vector3(61,0.2,60),Vector3(-25,0.2,82),Vector3(26,0.2,85)]
...
func _spawn_asset_to(parent: Node3D, path: String, pos: Vector3, rot: Vector3, visibility: float, layer: String, target_extent: float) -> Node3D:
 ...
 _set_visibility_recursive(node,visibility)
...
func _set_visibility_recursive(node: Node, distance: float) -> void:
 ...
 for child in node.get_children():
  _set_visibility_recursive(child,distance)
