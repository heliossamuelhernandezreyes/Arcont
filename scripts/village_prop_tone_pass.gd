...
# ART-PASS-14: neutralize bright provisional prop materials that dominate the
...
func _apply() -> void:
 ...
 for node: Node in _all_nodes(env):
  if not node is GeometryInstance3D:
   continue
  if _effective_layer(node,env) == "prop":
   (node as GeometryInstance3D).material_override = wood
   toned += 1
 set_meta("art_status","ART-PASS-14-PROP-TONE")
 set_meta("toned_prop_geometry",toned)
 set_meta("visual_reason","remove bright provisional crate dominance")
...
func _all_nodes(root: Node) -> Array[Node]:
 ...
 while i < out.size():
  ...
  for child: Node in current.get_children(): out.append(child)
  i += 1
 return out
