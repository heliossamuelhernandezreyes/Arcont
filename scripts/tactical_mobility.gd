...
func _try_traverse(min_height:float,max_height:float,duration:float,forward_landing:float,kind:String)->bool:
	...
	var reach:=vault_forward_distance+(0.28 if kind=="mantle" else 0.0)
	...
	var top_origin:=obstacle_point+Vector3.UP*(max_height+0.35)-forward*0.10
	...
	var top_point:Vector3=top_hit.get("position",obstacle_point);var obstacle_height:=top_point.y-player.global_position.y
	if obstacle_height<min_height or obstacle_height>max_height:return false
	var landing:=top_point+forward*forward_landing+Vector3.UP*0.05
...
func request_dodge(input_direction:=Vector3.ZERO) -> bool:
	...
	var direction:=input_direction;direction.y=0.0
...
func _try_cover_transfer(preferred:Vector3)->bool:
	...
	var world:=player.get_world_3d()
	if world==null:return false
	var direction:=preferred;direction.y=0.0
	...
	for distance in [2.0,3.0,4.0,cover_transfer_distance]:
		var probe:=player.global_position+direction*distance+Vector3.UP*0.7
		var rays=[direction,-direction,-player.global_transform.basis.z]
		for ray_dir in rays:
...
func handle_landing()->void:
	...
	if player.has_method("apply_damage"):
		var damage:=lerpf(7.0,34.0,minf(severity,1.0));player.call("apply_damage",damage,Vector3.ZERO,"left_leg" if randf()<0.5 else "right_leg",1.0)
	if severity>0.45:stance_changed.emit("hard_land")

func _update_traverse(delta:float)->void:
	traverse_elapsed+=delta;var t:=clampf(traverse_elapsed/maxf(traverse_duration,0.01),0.0,1.0);var smooth:=t*t*(3.0-2.0*t);var pos:=traverse_start.lerp(traverse_end,smooth);var arc:=0.52 if vaulting else 0.72;pos.y+=sin(t*PI)*arc;player.global_position=pos;player.velocity=Vector3.ZERO
