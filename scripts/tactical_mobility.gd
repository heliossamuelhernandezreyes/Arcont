...
func _physics_process(delta: float) -> void:
	...
	if sliding:
		slide_remaining=maxf(slide_remaining-delta,0.0);slide_velocity=maxf(slide_velocity-slide_friction*delta,0.0)
		if slide_remaining<=0.0 or slide_velocity<=1.0:_end_slide()
	if dodging:
...
func request_crouch_or_slide() -> bool:
	...
	if speed>=5.0 and player.is_on_floor() and not bool(player.get("in_cover")):
		if request_slide():return true
	toggle_crouch();return true

func request_slide() -> bool:
	if player==null or vaulting or mantling or dodging or sliding or not player.is_on_floor():return false
	var speed:=Vector2(player.velocity.x,player.velocity.z).length()
	if speed<5.0 or _leg_load()>=severe_leg_threshold or bool(player.get("in_cover")):return false
	var horizontal:=Vector3(player.velocity.x,0.0,player.velocity.z)
	if horizontal.length_squared()<0.01:return false
	sliding=true;crouched=true;slide_remaining=slide_duration*clampf(1.15-_leg_load()*0.25,0.58,1.0);slide_direction=horizontal.normalized();slide_velocity=slide_speed*clampf(1.0-_leg_load()*0.22,0.55,1.0);stance_changed.emit("slide");return true
...
func try_vault() -> bool:
	return _try_traverse(vault_height_min,vault_height_max,vault_duration,0.95,"vault")
...
func _try_traverse(min_height:float,max_height:float,duration:float,forward_landing:float,kind:String)->bool:
	if player==null or vaulting or mantling or sliding or dodging or not player.is_on_floor():return false
	var world:=player.get_world_3d()
	if world==null:return false
	var forward:Vector3=-player.global_transform.basis.z;forward.y=0.0;forward=forward.normalized()
	var origin:Vector3=player.global_position+Vector3.UP*0.55
	var reach:float=vault_forward_distance+(0.28 if kind=="mantle" else 0.0)
	var front_query:=PhysicsRayQueryParameters3D.create(origin,origin+forward*reach);front_query.exclude=[player.get_rid()]
	var front_hit:=world.direct_space_state.intersect_ray(front_query)
	if front_hit.is_empty():return false
	var obstacle_point:Vector3=front_hit.get("position",origin)
	var top_origin:Vector3=obstacle_point+Vector3.UP*(max_height+0.35)-forward*0.10
	var down_query:=PhysicsRayQueryParameters3D.create(top_origin,obstacle_point-Vector3.UP*0.15);down_query.exclude=[player.get_rid()]
	var top_hit:=world.direct_space_state.intersect_ray(down_query)
	if top_hit.is_empty():return false
	var top_point:Vector3=top_hit.get("position",obstacle_point);var obstacle_height:float=top_point.y-player.global_position.y
	if obstacle_height<min_height or obstacle_height>max_height:return false
	var landing:Vector3=top_point+forward*forward_landing+Vector3.UP*0.05
	var clearance:=PhysicsRayQueryParameters3D.create(landing+Vector3.UP*0.15,landing+Vector3.UP*1.65);clearance.exclude=[player.get_rid()]
	if not world.direct_space_state.intersect_ray(clearance).is_empty():return false
...
func request_dodge(input_direction:=Vector3.ZERO) -> bool:
	if player==null or dodging or sliding or vaulting or mantling or not player.is_on_floor() or dodge_cooldown_remaining>0.0:return false
	if _leg_load()>=severe_leg_threshold:return false
	...
	var distance:float=dodge_distance*clampf(1.0-_leg_load()*0.25,0.52,1.0);dodge_speed=distance/maxf(dodge_remaining,0.05);stance_changed.emit("dodge");return true
...
func _try_cover_transfer(preferred:Vector3)->bool:
	...
	for distance_value:float in [2.0,3.0,4.0,cover_transfer_distance]:
		var probe:Vector3=player.global_position+direction*distance_value+Vector3.UP*0.7
		var rays:Array[Vector3]=[direction,-direction,-player.global_transform.basis.z]
		for ray_dir:Vector3 in rays:
			var query:=PhysicsRayQueryParameters3D.create(probe,probe+ray_dir.normalized()*1.2);query.exclude=[player.get_rid()]
...
func movement_speed_multiplier()->float:
	...
	if sliding or dodging:return 1.0
	return crouch_speed_multiplier if crouched else 1.0
func apply_motion_override()->void:
	if player==null:return
	if sliding:player.velocity.x=slide_direction.x*slide_velocity;player.velocity.z=slide_direction.z*slide_velocity
	elif dodging:player.velocity.x=dodge_direction.x*dodge_speed;player.velocity.z=dodge_direction.z*dodge_speed
func blocks_normal_movement()->bool:return vaulting or mantling
func blocks_jump()->bool:return vaulting or mantling or sliding or dodging
...
func handle_landing()->void:
	var impact:float=absf(minf(last_vertical_speed,0.0));last_vertical_speed=0.0
	if impact<=safe_fall_speed:return
	var severity:float=clampf((impact-safe_fall_speed)/maxf(hard_fall_speed-safe_fall_speed,0.1),0.0,1.5)
...
func _update_traverse(delta:float)->void:
	traverse_elapsed+=delta;var t:float=clampf(traverse_elapsed/maxf(traverse_duration,0.01),0.0,1.0);var smooth:float=t*t*(3.0-2.0*t);var pos:Vector3=traverse_start.lerp(traverse_end,smooth);pos.y+=sin(t*PI)*(0.52 if vaulting else 0.72);player.global_position=pos;player.velocity=Vector3.ZERO
	if t>=1.0:vaulting=false;mantling=false;stance_changed.emit("crouch" if crouched else "stand")
func _end_slide()->void:
	if not sliding:return
	sliding=false;slide_remaining=0.0;slide_velocity=0.0;stance_changed.emit("crouch")
func _end_dodge()->void:
	if not dodging:return
	dodging=false;dodge_remaining=0.0;dodge_speed=0.0;stance_changed.emit("stand")
...
func _update_stance_visual(delta:float)->void:
	...
	if body_visual:
		var target_body_y:=base_body_y-crouch_visual_drop*crouch_alpha;body_visual.position.y=lerpf(body_visual.position.y,target_body_y,minf(delta*12.0,1.0));var target_pitch:=-12.0 if sliding else (-8.0 if dodging else (-4.0 if crouched else 0.0));body_visual.rotation_degrees.x=lerpf(body_visual.rotation_degrees.x,target_pitch,minf(delta*10.0,1.0))
	if camera_rig:
...
func _leg_load()->float:
	...
	var injuries:=data as Dictionary;return float(injuries.get("left_leg",0.0))+float(injuries.get("right_leg",0.0))
