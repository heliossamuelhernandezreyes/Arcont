...
static func build(parent: Node3D, world_pos: Vector3, yaw_deg: float, archetype: int, materials: Dictionary) -> Node3D:
 ...
 var width := [8.8, 10.2, 9.6, 10.8, 8.4, 10.6, 11.2, 9.8][archetype % 8]
 var depth := [7.2, 8.2, 8.6, 8.0, 7.5, 8.8, 8.4, 9.0][archetype % 8]
 var floors := 2 if archetype % 8 in [4, 5] else 1
 var floor_h := 2.85
 var body_h := floor_h * floors
 ...
 var door_x := -width * 0.18 if archetype % 2 == 0 else width * 0.17
...
static func _front_facade(root: Node3D, width: float, body_h: float, floor_h: float, floors: int, door_x: float, wall: Material, wood: Material, dark: Material, glass: Material) -> void:
 var z := -float(root.get_meta("dummy_depth", 0.0))
 ...
 var door_w := 1.05
 var door_h := 2.15
 var side_w := maxf(0.5, (width - door_w) * 0.5)
 ...
 for wx in [-width * 0.33, width * 0.31]:
  if absf(wx - door_x) < 1.15:
   continue
  _window(root, Vector3(wx, window_y, z - 0.09), glass, wood)
 if floors > 1:
  for wx in [-width * 0.29, 0.0, width * 0.29]:
   _window(root, Vector3(wx, floor_h + 1.55, z - 0.09), glass, wood)
...
static func _porch(root: Node3D, width: float, depth: float, wood: Material, roof: Material) -> void:
 var porch_w := minf(width * 0.72, 7.2)
 var front_z := -depth * 0.5 - 1.15
 _piece(root, "PorchDeck", Vector3(0, 0.56, front_z), Vector3(porch_w, 0.18, 2.15), wood)
 for x in [-porch_w * 0.43, porch_w * 0.43]:
  _piece(root, "PorchPost", Vector3(x, 1.72, front_z - 0.65), Vector3(0.18, 2.35, 0.18), wood)
