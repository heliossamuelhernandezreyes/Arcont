...
func _process(delta:float)->void:
 ...
 if execute_timer>0.0:
  ...
  if execute_timer<=0.0:
   execute_target=null
   execution_window_changed.emit(false)

func begin_guard(screen_position:=Vector2.ZERO)->void:
 if attack_timer>0.0:return
 guarding=true
 guard_line="mid"
 guard_started=Time.get_ticks_msec()/1000.0
 input_origin=screen_position
 guard_changed.emit(guard_line)

func update_guard_drag(current:Vector2)->void:
 if not guarding:return
 var dy:=current.y-input_origin.y
 var next_line:="mid"
...
func _attack_line(line:String)->bool:
 ...
 attack_visual.emit(line,profile)
 var hit:=_melee_trace(reach)
 if hit.is_empty():
  melee_feedback.emit(line.to_upper())
  return true
 var target:Node=hit.get("collider") as Node
 if target==null:return true
 var point:Vector3=hit.get("position",target.global_position if target is Node3D else player.global_position)
 var direction:Vector3=-camera.global_transform.basis.z
 if target.has_method("receive_melee_attack"):
  target.call("receive_melee_attack",line,profile,player,direction)
 ...
 melee_feedback.emit("STRIKE "+line.to_upper())
 return true
...
func receive_melee_attack(line:String,attacker:Node,damage:=35.0)->Dictionary:
 ...
 var elapsed:=Time.get_ticks_msec()/1000.0-guard_started
 var perfect:=elapsed<=guard_parry_window
 if not perfect:
  parry_visual.emit(line,"block")
  return {"result":"block","damage":damage*0.28}
 if line=="mid":
  _push_apart(attacker)
  parry_visual.emit(line,"mid_parry")
  melee_feedback.emit("PARRY · SPACE")
  return {"result":"mid_parry","damage":0.0,"advantage":mid_parry_advantage}
 execute_target=attacker
 execute_timer=execute_window
 execution_window_changed.emit(true)
 parry_visual.emit(line,"parry")
 melee_feedback.emit("PARRY · EXECUTE")
 return {"result":"parry","damage":0.0,"execute":true}
