...
func _process(delta:float)->void:
 ...
 if execute_timer>0.0:
  ...
  if execute_timer<=0.0:
   execute_target=null;execution_window_changed.emit(false)
...
func _attack_line(line:String)->bool:
 ...
 if target.has_method("receive_melee_attack"):
  target.call("receive_melee_attack",line,_profile_name(),player,direction)
...
func receive_melee_attack(line:String,attacker:Node,damage:=35.0)->Dictionary:
 ...
 if not perfect:return {"result":"block","damage":damage*0.28}
 if line=="mid":
  _push_apart(attacker);melee_feedback.emit("PARRY · SPACE");return {"result":"mid_parry","damage":0.0,"advantage":mid_parry_advantage}
 execute_target=attacker;execute_timer=execute_window;execution_window_changed.emit(true);melee_feedback.emit("PARRY · EXECUTE");return {"result":"parry","damage":0.0,"execute":true}
...
func _execute_target()->bool:
 ...
 if target.has_method("melee_execute"):target.call("melee_execute",player,_profile_name())
...
func _melee_trace(distance:float)->Dictionary:
 ...
 var origin:=camera.global_position;var forward:=-camera.global_transform.basis.z
 var query:=PhysicsRayQueryParameters3D.create(origin,origin+forward*distance);query.exclude=[player.get_rid()];query.collide_with_areas=false
