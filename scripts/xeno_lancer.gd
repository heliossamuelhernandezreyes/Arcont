...
func _physics_process(delta: float) -> void:
	...
	elif has_visual_contact and global_position.distance_to(player.global_position)<=fire_range and fire_timer<=0.0:
		_begin_charge()
...
func _update_awareness(delta: float) -> void:
	...
	if awareness and awareness.has_method("recent_sound_for"):
		...
		if not heard.is_empty() and float(heard.get("age",99.0)) < 0.4:
			...
			memory_remaining = maxf(memory_remaining,5.5)
...
func _choose_tactic() -> void:
	...
	if not has_visual_contact:
		tactical_target = last_known_position
		has_tactical_target = true
		return
	if role == "suppress":
		var cover := TacticalAI.best_cover(self,player,preferred_distance,30.0)
		if cover != Vector3.INF and global_position.distance_to(player.global_position)>fire_range*0.82:
...
func _velocity_to(target: Vector3) -> Vector3:
	var waypoint := _next_navigation_point(target)
...
func _next_navigation_point(target: Vector3) -> Vector3:
	if navigation and navigation.has_method("next_waypoint"):
		return navigation.next_waypoint(global_position,target,1.15)
	return target

func _begin_charge() -> void:
	charging=true;charge_remaining=charge_time;velocity.x*=0.35;velocity.z*=0.35
	core_light.light_color=Color(0.86,0.12,1.0);core_light.light_energy=2.2
