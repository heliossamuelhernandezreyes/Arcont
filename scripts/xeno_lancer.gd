...
func _ready()->void:
	health=max_health;player=get_tree().get_first_node_in_group("player") as Node3D
	var scene:=get_tree().current_scene
	if scene:awareness=scene.get_node_or_null("AwarenessDirector");navigation=scene.get_node_or_null("NavigationGraph")
	add_to_group("xeno_enemy");add_to_group("tactical_enemy");add_to_group("enemies_active");role=TacticalAI.squad_role(self);decision_timer=randf_range(0.1,decision_interval)
func _physics_process(delta:float)->void:
	if not active:return
	if emp_remaining>0.0:
		emp_remaining=maxf(emp_remaining-delta,0.0);charging=false;velocity.x=move_toward(velocity.x,0.0,9.0*delta);velocity.z=move_toward(velocity.z,0.0,9.0*delta);core_light.light_energy=0.25
		if not is_on_floor():velocity.y-=gravity*delta
		move_and_slide()
		if emp_remaining<=0.0:core_light.light_energy=1.4
		return
	...
	if decision_timer<=0.0 and not charging:decision_timer=decision_interval+randf_range(-0.2,0.3);_choose_tactic()
	var desired:=Vector3.ZERO if charging else _movement_velocity();velocity.x=move_toward(velocity.x,desired.x,7.0*delta);velocity.z=move_toward(velocity.z,desired.z,7.0*delta)
	var face_target:=player.global_position if has_visual_contact else last_known_position;var flat:=face_target-global_position;flat.y=0.0
...
func apply_emp(duration:float)->void:
	if not active:return
	emp_remaining=maxf(emp_remaining,duration);charging=false;charge_remaining=0.0;fire_timer=maxf(fire_timer,duration);core_light.light_color=Color(0.18,0.65,1.0);core_light.light_energy=0.25
func _update_awareness(delta:float)->void:
	has_visual_contact=TacticalAI.has_line_of_sight(self,player,1.35,0.72)
	if has_visual_contact:last_known_position=player.global_position;memory_remaining=visual_memory_seconds;awareness_state="contact";return
	...
	if awareness and awareness.has_method("recent_sound_for"):
		...
		if not heard.is_empty() and float(heard.get("age",99.0))<0.4:last_known_position=heard.get("position",player.global_position);memory_remaining=maxf(memory_remaining,5.5);awareness_state="investigate";return
	if awareness and awareness.has_method("shared_intel_for"):
		var intel:Dictionary=awareness.shared_intel_for("armed")
		if not intel.is_empty() and float(intel.get("age",99.0))<0.45:last_known_position=intel.get("position",player.global_position);memory_remaining=maxf(memory_remaining,6.0);awareness_state="investigate";return
	if memory_remaining>0.0:awareness_state="search"
	else:awareness_state="idle";has_tactical_target=false
func _choose_tactic()->void:
	if player==null:return
	role=TacticalAI.squad_role(self)
	if awareness_state=="idle":has_tactical_target=false;return
	if not has_visual_contact:tactical_target=last_known_position;has_tactical_target=true;return
	if role=="suppress":
		...
		return
	var side:=-1.0 if role=="flank_left" else 1.0;var flank:=TacticalAI.flank_point(self,player,side,preferred_distance);var cover:=TacticalAI.best_cover_near(self,player,flank,preferred_distance,32.0);tactical_target=cover if cover!=Vector3.INF else flank;has_tactical_target=true
func _movement_velocity()->Vector3:
	...
	return Vector3.ZERO
func _velocity_to(target:Vector3)->Vector3:
	var waypoint:=_next_navigation_point(target);var delta:=waypoint-global_position;delta.y=0.0;return delta.normalized()*move_speed if delta.length_squared()>0.01 else Vector3.ZERO
func _next_navigation_point(target:Vector3)->Vector3:
	if navigation and navigation.has_method("next_waypoint"):return navigation.next_waypoint(global_position,target,1.15)
	return target
func _begin_charge()->void:charging=true;charge_remaining=charge_time;velocity.x*=0.35;velocity.z*=0.35;core_light.light_color=Color(0.86,0.12,1.0);core_light.light_energy=2.2
...
func apply_hit(_hit_point:Vector3,direction:Vector3,amount:float,_weapon_type:="shotgun",impact_force:=1.0)->void:
	...
	if health<=0.0:active=false;remove_from_group("enemies_active");remove_from_group("tactical_enemy");died.emit(self);queue_free()
